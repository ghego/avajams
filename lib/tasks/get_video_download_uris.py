...
def convert_embed_to_watch(url):
  components = r['urls'].replace('embed', 'watch').split("/")
...
if __name__ == '__main__':
  ...
  parser.add_argument("--index", help="Test a saved classifier", type=int, default=0)
  ...
  with open(os.environ['PROJECT_ROOT'] + "/data/video_download_urls.csv", 'w') as csvfile:
    ...
    for r in rows[args.index:]:
      ...
      if response.get('redirect'):
        raise Exception("|ERROR| Captcha validation requested |{}|".format(response))
